...
- **Closing CTA block** — badge pill, big two-line headline (second line
  muted), subcopy, actions. Ramblings captures an email for a waitlist; zineps
  has no waitlist, so the form becomes the reference's own CTAs: `Start your
  trial` (primary, mint) and `Talk to sales` (secondary, bordered).
...
- **Link columns** — Products / Company / Contact, straight from the reference
  footer, plus the © line and privacy/terms links.
